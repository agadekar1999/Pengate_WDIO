exports.config = {
    ...
    capabilities: [{
        ...
        browserName: 'chrome',
        'goog:chromeOptions': {
            args: ['--headless=new', '--disable-gpu', '--window-size=1920,1080', '--no-sandbox']
        }
    }],
    logLevel: 'info',
    bail: 0,
    ...
    afterTest: async function(test, context, { error, result, duration, passed, retries }) {
        if (!passed) {
            ...
            addAttachment('Screenshot on Failure', Buffer.from(await browser.takeScreenshot(), 'base64'), 'image/png');
        }
    },
}
